...
list_of_snapshot_trans = [
    ...
    # FastICA(n_components=9, tol=0.3, max_iter=1000, random_state=42),
    # SelectKBest(k=150),
    # SelectKBest(k=120),
    PCA(5),
    SelectKBest(k=5),
                    ...
                    for sen_type, sen_df in df.groupby(["Sentence_type"]):
                        ...
                        try:
                            ...
                            fitted_classification_pipeline, y_pred, y_true, y_proba, test_acc, train_acc = train_classifiction(classification_pipeline, sen_df,
                                                                                                                               ...
                                                                                                                               X_cols = ET_Features, y_col = Y_COL,
                                                                                                                               mode = mode
                                                                                                                               )
                            et = time.time()
...
def analyse_config_results(analysis_artifacts_output_dir, config_df, df, pipeline_idx, method, et_scale_col, title_suf,
        ...
        try:
            ...
            for subj_i, subj_num in enumerate(cv_test_subjects):
                ...
                row = [cv_num, subj_num, per_subject_unified_prediction[subj_i], subj_label, subj_success] + \
                                                 [cv_df[cv_df.sentence_type==x]['test_acc'].iloc[0] for x in ['A','B','C','D']]
                config_rows.append(row)
        except Exception as e:
            print("=== Failed for " , cv_num)
    ...
    subj_agg_res_df = pd.DataFrame(data = config_rows, columns = ["cv_num", "test_subject", "prediction","true_label",
                                                                  "subj_success"] + [f"{x}_success" for x in ['A','B','C','D']])
...
desc_suffix = "7-8_iterall__ET_FEATURES_"
...
for is_perm_iter in [False, True]:
    ...
    # if run_multiprocess:
        ...
        df_fn = "/Users/orenkobo/Desktop/PhD/HebLingStudy/ts_data/Artifacts2/df_new_distilled_unsegmented_alldata_new_FINAL_paparanalysis_withETLingFeatures.csv"
        print(f"{datetime.datetime.now()} Read df from {df_fn} col is {et_scale_col}")
        all_subjs_per_subject_settype_mean_df, timepoint_cols ,df = get_timecols_df(fn =df_fn, scale_col = et_scale_col, return_df=True)
        df_to_use = df[ET_Features + ['Subject','Sentence_type','set_num',
                                      'phq_score','phq_group']].dropna().reset_index(drop=True)
        print(f"{datetime.datetime.now()} Got df with shape {df_to_use.shape} , {len(timepoint_cols)} time cols")
