...
list_of_snapshot_trans = [
    ...
    FastICA(n_components=12, tol=0.3, max_iter=500, random_state=42),
    # SelectKBest(k=150),
    # SelectKBest(k=120),
    # SelectKBest(k=90),
    # SelectKBest(k=60),
...
for is_perm_iter in [False, True]:
    ...
    # if run_multiprocess:
        ...
        for sen_type,sen_type_df in all_subjs_per_subject_settype_mean_df.groupby(["Sentence_type"]):
            print(f"{datetime.datetime.now()} Got df with shape {sen_type_df.shape} , {len(timepoint_cols)} time cols (type {sen_type}")
            for override_cutoff in CUTOFFS:
                for method in METHODS:
                    try:
                        p = "_".join([str(x) for x in override_cutoff])
                        print(f"{datetime.datetime.now()}: ================= (c={c}) Work on {p}, {sen_type} , {method}, {et_scale_col}, {is_perm_iter_prefix} ================= ")
                        df = apply_phq_cutoff(sen_type_df,
                        ...
                        snapshot_per_fold_per_cond_per_confid_res_df["sentence_type"] = sen_type
                        ...
                        snapshot_per_model_subj_res_df["sentence_type"] = sen_type

                        snapshot_per_fold_per_cond_per_confid_res_df.to_csv(f"{mode_artifacts_output_dir}/{p}{suffix}{method}_snapshot_per_fold_per_cond_per_config_res_df_PER_SENTENCE_TYPE.csv", index = False)
                        snapshot_per_model_subj_res_df.to_csv(f"{mode_artifacts_output_dir}/{p}{suffix}{method}_{sen_type}_snapshot_per_model_subj_res_df_PER_SENTENCE_TYPE.csv", index = False)
                        ...
                        print("Error : ", e)
                        errors.append([e, (is_perm_iter, et_scale_col, sen_type, override_cutoff, method)])
print(f"{datetime.datetime.now()}: Got {len(errors)} errors : {errors}")
all_res_df = pd.concat(all_res_dfs, axis=0)
all_res_per_cv_df = pd.concat(all_res_per_cv, axis=0)
all_res_df.to_csv(f"{analysis_artifacts_dir}/{script_start_timestamp}/{script_start_timestamp}_all_res_df{suffix}_PER_SENTENCE_TYPE.csv", index = False)
all_res_per_cv_df.to_csv(f"{analysis_artifacts_dir}/{script_start_timestamp}/{script_start_timestamp}_all_res_per_cv_df{suffix}_PER_SENTENCE_TYPE.csv", index = False)
with open(f"{analysis_artifacts_dir}/{script_start_timestamp}_description_PER_SENTENCE_TYPE.txt", "w") as text_file:
    text_file.write(str_output)

if PERM_RUN:
    print(f"{datetime.datetime.now()}: Saved new perm benchmark to {perm_benchmark_df_fn}")
    all_res_df.to_csv(perm_benchmark_df_fn + "__PER_SENTENCE_TYPE.csv", index = False)
else:
    try:
        ...
        merged_out_fn = f"{analysis_artifacts_dir}/{script_start_timestamp}/{script_start_timestamp}_all_res_df_with_perm{suffix}_PER_SENTENCE_TYPE.csv"
